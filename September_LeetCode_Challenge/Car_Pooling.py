...
# My Solution 
# O(max(N,1001)) Time and O(1) Space
class Solution:
    def carPooling(self, trips: List[List[int]], capacity: int) -> bool:
        
        trips= sorted(trips, key=lambda x: x[1])
        
        num_people = [0 for i in range(1001)]
        
        for trip in trips:
            
            num_people[trip[1]] += trip[0]
            num_people[trip[2]] -= trip[0]
        
        
        for element in num_people:
            capacity -= element
...
# Given Solution - Basically same with different structure
