...
# My Solution
# O(1) Time as number of rows is fixed, O(1) Space for the same reason
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        ...
        for row in range(query_row+1):
            for col in range(row+1):
                ...
                overflow = (M[row][col] - 1) / 2
                
                if overflow>0:
                    M[row+1][col]   += overflow
                    M[row+1][col+1] += overflow
                    
        
        return min(1, M[query_row][query_glass])
