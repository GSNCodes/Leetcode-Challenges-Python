...
and sum the result of the division. Find the smallest divisor such that the result 
...
# O(n-max) Time where n-max is the maximum value in the array and O(1) Space
class Solution:
    def smallestDivisor(self, nums: List[int], threshold: int) -> int:
        
        def helper(val):
            
            return sum([math.ceil(x/val) for x in nums])
