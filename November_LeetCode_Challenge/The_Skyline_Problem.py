...
the x coordinates of the left and right edge of the ith building, respectively, 
...
def getSkyline(self, buildings: List[List[int]]) -> List[List[int]]:
        r,b,p,h = [],set(),collections.defaultdict(list),[]
        for i,v in enumerate(buildings):
            p[v[0]].append((v[2],1,i))
            p[v[1]].append((v[2],0,i))
        for i in sorted(p.keys()):
            for j in p[i]:
                if not j[1]: b.add(j[2])
                else: heapq.heappush(h,(-j[0],j[2]))
            while h and h[0][1] in b: heapq.heappop(h)
            if not h: r.append([i,0])
            elif not r or -r[-1][1] != h[0][0]: r.append([i,-h[0][0]])
        return r